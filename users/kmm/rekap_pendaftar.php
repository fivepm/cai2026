...
if (!isset($_SESSION['logged_in']) || substr($_SESSION['user_role'], 0, 5) != 'ketua') {
    $_SESSION['message'] = ['type' => 'error', 'text' => 'Anda tidak memiliki hak akses.'];
    header("Location: ../../index.php");
    exit();
}
if ($_SESSION['user_kelompok'] == 'bintaran') {
    $user_kelompok = 'Bintaran';
} else if ($_SESSION['user_kelompok'] == 'gedongkuning') {
    $user_kelompok = 'Gedongkuning';
} else if ($_SESSION['user_kelompok'] == 'jombor') {
    $user_kelompok = 'Jombor';
} else if ($_SESSION['user_kelompok'] == 'sunten') {
    $user_kelompok = 'Sunten';
}
...
$base_query = "SELECT * FROM ({$sql}) AS semua_pendaftar WHERE kelompok='$user_kelompok'";
$params = [];
$types = '';
...
$result_peserta = $conn->query("SELECT jenis_kelamin, kelompok, COUNT(id) as jumlah FROM peserta WHERE kelompok='$user_kelompok' GROUP BY jenis_kelamin, kelompok");
if ($result_peserta) {
...
$result_izin = $conn->query("SELECT jenis_kelamin, kelompok, COUNT(id) as jumlah FROM izin WHERE kelompok='$user_kelompok' GROUP BY jenis_kelamin, kelompok");
if ($result_izin) {
...
<body class="bg-gray-100 font-sans">
    ...
    <!-- Konten Utama -->
    <div x-data="{ sidebarOpen: false, isFilterModalOpen: false }" @keydown.escape.window="isFilterModalOpen = false" class="flex-1 flex flex-col overflow-hidden">
        <header class="flex items-center justify-between px-6 py-4 bg-white border-b-4 border-red-600">
            ...
            <div x-data="{ dropdownOpen: false }" class="relative">
                ...
                <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="absolute right-0 z-20 w-48 py-2 mt-2 bg-white rounded-md shadow-xl" x-transition>
                    <a href="logout.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-red-500 hover:text-white">
                        Logout
                    </a>
                </div>
            </div>
        ...
        <main class="flex-1 p-6 overflow-x-hidden overflow-y-auto bg-gray-100">
            <h1 class="text-center text-3xl font-semibold text-gray-800">
                Rekapitulasi Pendaftar - <?php echo $user_kelompok; ?>
            </h1>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            foreach ($pendaftar_list as $pendaftar): ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        <?php if (!empty($pendaftar['metode_pembayaran'])): ?>
                                            <?php echo htmlspecialchars($pendaftar['metode_pembayaran']); ?>
                                        <?php else: echo '-';
                                        endif; ?>
                                    </td>
                                    <!-- <td class="px-6 py-4">
                                        <?php if (!empty($pendaftar['file_izin'])): ?>
                                            <a href="<?php echo $upload_dir . htmlspecialchars($pendaftar['file_izin']); ?>" target="_blank" class="text-blue-600 hover:underline"><i class="fas fa-file-alt mr-1"></i>Lihat File</a>
                                        <?php else: echo '-';
                                        endif; ?>
                                    </td> -->
                    ...
                    <form action="rekap_pendaftar" method="GET" class="space-y-4">
                        ...
                        <!-- <div>
                            ...
                            <select id="kelompok" name="kelompok" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md">
                                ...
                                <option value="Gedonkuning" <?php if ($filter_kelompok == 'Gedonkuning') echo 'selected'; ?>>Gedonkuning</option>
