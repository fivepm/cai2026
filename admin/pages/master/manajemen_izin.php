...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    header("Location: admin?page=master/manajemen_izin");
    exit();
}
...
<div x-data="{ 
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">Manajemen Peserta Izin</h1>
        <button @click="isFilterModalOpen = true" class="px-4 py-2 font-semibold text-gray-700 bg-white rounded-lg shadow-sm hover:bg-gray-50 flex items-center">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <?php if ($izin['status'] == 'perlu_verifikasi'): ?>
                                        <form method="POST" action="" class="inline-flex space-x-2">
                                            ...
                                            <button type="submit" name="action" value="terima" class="px-3 py-1 text-sm text-white bg-green-500 rounded-lg hover:bg-green-600">Terima</button>
                                            <button type="submit" name="action" value="tolak" class="px-3 py-1 text-sm text-white bg-red-500 rounded-lg hover:bg-blue-600">Tolak</button>
                                        </form>
                                    <?php
                                    endif; ?>
                                    <form method="POST" action="" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data izin ini secara permanen?');">
                                        <input type="hidden" name="izin_id" value="<?php echo $izin['id']; ?>">
                                        <button type="submit" name="action" value="hapus" class="px-3 py-1 text-sm text-white bg-gray-600 rounded-lg hover:bg-gray-700">
    ...
    <div x-show="isFilterModalOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
        <div @click.away="isFilterModalOpen = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 mx-4">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-white">Filter Peserta Izin</h3>
            </div>
            <form action="admin.php" method="GET" class="space-y-4">
                ...
                <div><label class="block text-sm">Filter Status</label><select name="status" class="mt-1 w-full border-gray-300 rounded-lg">
                ...
                <div class="mt-6 flex justify-end space-x-4"><button type="button" @click="isFilterModalOpen = false" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">Batal</button><button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg">Terapkan Filter</button></div>
            </form>
        </div>
    </div>
    ...
    <div x-show="isSuratModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" x-cloak>
        <div @click.away="isSuratModalOpen = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-xl p-4 mx-4 relative">
            <button @click="isSuratModalOpen = false" class="absolute -top-3 -right-3 bg-blue-600 text-white rounded-full h-8 w-8 flex items-center justify-center">&times;</button>
